...
local REDIRECT_URI_FORMAT = "http://127.0.0.1:%d/callback"
...
local function get_auth_url(client_id, scopes, redirect_uri_port)
	local redirect_uri = string.format(REDIRECT_URI_FORMAT, redirect_uri_port)

	return string.format(AUTH_URL_FORMAT, client_id, table.concat(scopes, SPACE_URL_CHAR), redirect_uri)
end

---@param port integer
---@param with_auth_code_callback fun(code: string)
local function start_redirect_server(port, with_auth_code_callback)
...
local function save_auth_tokens(
	...
	local redirect_uri_param = redirect_uri_port
			and "&redirect_uri=" .. string.format(REDIRECT_URI_FORMAT, redirect_uri_port)
		or ""
...
---@param redirect_uri_port integer
local function exchange_code_for_token(client_id, client_secret, auth_code, redirect_uri_port)
	save_auth_tokens(client_id, client_secret, "authorization_code", "code", auth_code, redirect_uri_port)
end

---@param client_id string
---@param client_secret string
---@param refresh_token string
local function refresh_tokens(client_id, client_secret, refresh_token)
	save_auth_tokens(client_id, client_secret, "refresh_token", "refresh_token", refresh_token)
end
...
---@param scopes string[]
---@param redirect_uri_port integer
local function authenticate(client_id, client_secret, scopes, redirect_uri_port)
...
M.authenticate = function(client_id, client_secret, scopes, redirect_uri_port)
	...
	authenticate(client_id, client_secret, state.scopes, state.redirect_uri_port)
	await_tokens_refresh()
	---@diagnostic disable-next-line return-type-mismatch
	return load_tokens()
end
